...
const Pagination = ({ currentPage, totalPages, onPageChange }) => {
  ...
  const [visiblePageNumbers, setVisiblePageNumbers] = useState(pageNumbers.slice(0, 7));
  ...
  const handleNextClick = () => {
    setCurrentIndex((prevIndex) => Math.min(prevIndex + 7, pageNumbers.length - 1));
  };
  ...
  return (
    <div className={style.pagination}>
      ...
      <button
        className={currentIndex >= pageNumbers.length - 7 ? style.disabled : ''}
        onClick={handleNextClick}
        disabled={currentIndex >= pageNumbers.length - 7}
      >
        &gt;
      </button>
      <button
